  ...
  return (
    <>
     <section className="max-container padding-container flex flex-col gap-20 py-16 md:gap-28 lg:py-40 relative" itemScope itemType="http://schema.org/WebPage" >
       ...
       <div className="relative pl-5 z-10 bg-slate-400 flex flex-col  ">
         <div className="absolute inset-0 bg-hero-img opacity-50 z-[-1] bg-no-repeat"  style={{ backgroundImage: `url('KeyArt.jpg')`, backgroundSize: ' auto 100%', backgroundRepeat: 'no-repeat' }}/>
         <ul ref={ulRef} itemProp="mainEntity" itemScope itemType="http://schema.org/ItemList">
           {FEATURES.map((feature, index) => (
             ...
             {isEvenIndex(index)&&(
               ...
               key={feature.id}
               id={feature.id}
               className={`mt-0 md:mt-[${feature.id === excludedFeatureId ? '0' : '30rem'}] feature flex flex-col w-[100%] items-center pr-4 pt-4
